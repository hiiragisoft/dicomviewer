...
//    private static Logger log = LoggerFactory.getLogger(DicomInputStream.class);
    ...
    private void setVallen(int value)
    {
//    	if(value > 1000)
//    	{
//    		System.out.println("DicomInputStream vallen: " + value);
//    		Thread.dumpStack();
//    	}
    	vallen = value;
    }
...
//                        .warn("Missing (0002,0010) Transfer Synatx in File Meta Information");
        ...
        if (TagUtils.hasVR(tag) && ts.explicitVR()) {
            try {
            	int code = ((header[4] & 0xff) << 8) | (header[5] & 0xff);
            	//System.out.println("VR.valueOf " + Integer.toHexString(code) + " tag: " + Integer.toHexString(tag));
...
//                        + " - assume " + vr);
            ...
            if (vr.explicitVRHeaderLength() == 8) {
                int vallen = ts.bigEndian() ? ByteUtils.bytesBE2ushort(header, 6): ByteUtils.bytesLE2ushort(header, 6);
...
//                    }
        ...
        int vallen2 = ts.bigEndian() ? ByteUtils.bytesBE2int(header, 4) : ByteUtils.bytesLE2int(header, 4);
        setVallen(vallen2);
        return tag;
    }
...
//                    log.warn("Unexpected EOF - treat as (FFFE,E0DD) Sequence Delimitation Item");
            ...
            if (TagUtils.hasVR(tag) && (vr == null || vr == VR.UN))
            {
                ...
                //System.out.println("(477)tag: " + Integer.toHexString(tag));
                vr = attrs.vrOf(tag);
            }
...
//                        tagpos + " - try to skip length");
        ...
        default:
            ...
            {
                DicomElement a = vr == VR.SQ ? attrs.putSequence(tag) : attrs.putFragments(tag, vr, ts.bigEndian());
                readItems(a, vallen);
            }
            else
            {
                DicomElement a = attrs.putBytes(tag, vr, readBytes(vallen), ts.bigEndian());
                if (tag == 0x00020000)
                {
